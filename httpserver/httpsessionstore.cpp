...
{
    connect(&cleanupTimer, &QTimer::timeout, this, &HttpSessionStore::sessionTimerEvent, Qt::UniqueConnection);
...
QByteArray HttpSessionStore::getSessionId(HttpRequest& request, HttpResponse& response)
{
    ...
    if (sessionId.isEmpty()) sessionId=request.getCookie(cookieName);
...
HttpSession HttpSessionStore::getSession(HttpRequest& request, HttpResponse& response, bool allowCreate)
{
    ...
    if (!sessionId.isEmpty())
    {
        ...
        if (!session.isNull())
        {
            ...
            response.setCookie(HttpCookie(cookieName, session.getId(), expirationTime / 1000, cookiePath, cookieComment, cookieDomain, false, false, "Lax"));
